...
| `VHAL_ESP32_DSI` | MIPI DSI Display |
| `VHAL_ESP32_PPA` | Pixel Processing Accelerator |
| `VHAL_ESP32_LDO` | LDO Voltage Regulator |
...
| Define | Description |
|--------|-------------|
| `VHAL_SYSTEM_CONSOLE` | Enable console support: `System::console`, `SetWriteHandler`/`SetReadHandler`. |
| `VHAL_SYSTEM_CONSOLE_RUNTIME` | Enable newlib `_write`/`_read` stubs so `printf()` works through VHAL console. Requires `VHAL_SYSTEM_CONSOLE`. Use for bare-metal platforms; platforms like ESP-IDF provide their own stubs and do not need this. |
