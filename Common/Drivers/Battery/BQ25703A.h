...
public:
	...
	void SetChargeCurrent(uint16 data){ 
		chargeCurrentRegister = data; 
	}


	bool AdapterIsPresent() { 
		return adapterPresentStatus; 
	}


public:
	Status::statusType SetChargeOption0(uint16 data){ 
		return WriteReg(0x00, data); 
	}
...
private:
	...
	Status::info<uint16> ReadReg(uint8 addrress) {
		uint16 data;
		auto status = i2c->ReadByteArray(0xd6, addrress, 1, (uint8*)&data, 2);
