...
namespace OS {
	...
	struct RtosWrapper {
		...
		inline static tEventBits wWaitEvent(tEventHandle const &eventHandle, const tEventBits mask, tTime timeOut, OS::EventMode mode) {
			BaseType_t xWaitForAllBits = pdFALSE;
			if (mode == OS::EventMode::waitAnyBits) {
